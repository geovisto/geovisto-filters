...
class MapFiltersManager extends MapDomainArrayManager<IMapFilterOperation> implements IMapFilterManager {
     ...
     * @param filterRules 
     */
    public filterData(dataManager: IMapDataManager, dataRecords: IMapData, filterRules: IMapFilterRule[]): IMapData {
        ...
        for(let i = 0; i < dataRecords.length; i++) {
            ...
            for(let j = 0; j < filterRules.length; j++) {
                values = dataManager.getDataRecordValues(dataDomains[j], dataRecords[i]);
                if(values.length == 1) {
